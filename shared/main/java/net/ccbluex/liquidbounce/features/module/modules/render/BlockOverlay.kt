...
class BlockOverlay : Module() {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        val color = if (colorRainbow.get()) RenderUtils.testRainbow(0, 1f, 0.9f, 5.0) else Color(colorRedValue.get(),
    ...
    fun onRender2D(event: Render2DEvent) {
        if (infoValue.get()) {
            ...
            classProvider.getGlStateManager().resetColor()
            Fonts.rise.drawString(info, scaledResolution.scaledWidth / 2f, scaledResolution.scaledHeight / 2f + 7f, Color.WHITE.rgb, false)
        }
    }
}