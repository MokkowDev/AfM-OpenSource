...
class KillAura : Module() {
    ...
    private val cooldownValue = FloatValue("Cooldown-(1.12 ver)", 1f, 0f, 1f)

    // Range
    private val rangeValue = FloatValue("Range", 3.7f, 1f, 8f)
    private val throughWallsRangeValue = FloatValue("WallsRange", 3f, 0f, 8f)
    private val rangeSprintReducementValue = FloatValue("SprintReducement", 0f, 0f, 0.4f)
    ...
    // AutoBlock
    private val autoBlockValue = ListValue("AutoBlock", arrayOf("Off", "NCP", "AfterTick", "Redesky", "Vanilla", "Vanilla2"), "Vanilla")
    private val interactAutoBlockValue = BoolValue("InteractAutoBlock", true)
    private val blockRate = IntegerValue("Block%", 100, 1, 100)
    ...
    // Turn Speed
    private val maxTurnSpeed: FloatValue = object : FloatValue("MaxRotationSpeed", 180f, 0f, 180f) {
    ...
    private val minTurnSpeed: FloatValue = object : FloatValue("MinRotationSpeed", 180f, 0f, 180f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
    ...
    fun onMotion(event: MotionEvent) {
        if (event.eventState == EventState.POST) {
            ...
            if(autoBlockValue.get().equals("Vanilla2",true))
                startBlocking(currentTarget!!, hitable)
        }
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (markValue.get() && !targetModeValue.get().equals("Multi", ignoreCase = true))
            RenderUtils.drawPlatform(target, if (hitable) Color(100, 255, 100, 200) else Color(255, 0, 0, 200))
    ...
    private fun attackEntity(entity: IEntityLivingBase) {
        ...
        if (autoBlockValue.get().equals("NCP", true) && (thePlayer.isBlocking || canBlock))
