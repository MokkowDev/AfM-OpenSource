...
@ModuleInfo(name = "Antivoid", description = "Automatically setbacks you after falling to the void.", category = ModuleCategory.MOVEMENT)
class Antivoid : Module() {
    private val modeValue = ListValue("Mode", arrayOf("Vanilla", "Flag", "OnGround", "MotionTeleport-Flag"), "FlyFlag")
    private val maxFallDistance = IntegerValue("MaxFallDistance", 255, 2, 255)
        ...
        if (!thePlayer.onGround && !thePlayer.isOnLadder && !thePlayer.isInWater) {
            ...
            if (thePlayer.fallDistance - lastFound > maxDistanceWithoutGround.get()) {
                ...
                when (mode.toLowerCase()) {
                    ...
                    "onground" -> mc.netHandler.addToSendQueue(classProvider.createCPacketPlayer(true))
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        RenderUtils.glColor(Color(0, 255, 0, 90))
        ...
        RenderUtils.renderNameTag("${fallDist}m (take ~${max(0, fallDist - 3)} damage)", x + 0.5, y + 1.7, z + 0.5)
