...
class ESP : Module() {
    @JvmField
    val modeValue = ListValue("Mode", arrayOf("Box", "OtherBox", "WireFrame", "2D", "Real2D", "Outline"), "Box")
    ...
    private val shaderGlowRadius = FloatValue("Just useless value", 2.3f, 2f, 3f)
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        val shader = (if (mode.equals("shaderglow", ignoreCase = true)) GlowShader.GLOW_SHADER else null)
                ?: return
        val radius = if (mode.equals("shaderglow", ignoreCase = true)) shaderGlowRadius.get() else 1f
