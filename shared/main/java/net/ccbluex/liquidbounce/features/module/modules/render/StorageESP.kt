...
class StorageESP : Module() {
    private val modeValue = ListValue("Mode", arrayOf("Box", "OtherBox", "Outline", "2D", "WireFrame"), "Outline")
    private val chestValue = BoolValue("Chest", true)
    private val enderChestValue = BoolValue("Ender-Chest", false)
    private val chestred = IntegerValue("Chest-Red", 0, 0, 255)
    private val chestgreen = IntegerValue("Chest-Green", 66, 0, 255)
    private val chestblue = IntegerValue("Chest-Blue", 255, 0, 255)
    private val chestalpha = IntegerValue("Chest-Alpha", 80, 0, 200)
    private val echestred = IntegerValue("EnderChest-Red", 81, 0, 255)
    private val echestgreen = IntegerValue("EnderChest-Green", 10, 0, 255)
    private val echestblue = IntegerValue("EnderChest-Blue", 205, 0, 255)
    private val echestalpha = IntegerValue("EnderChest-Alpha", 80, 0, 200)

    val r = chestred.get()
    val g = chestgreen.get()
    val b = chestblue.get()
    val a = chestalpha.get()
    val er = echestred.get()
    val eg = echestgreen.get()
    val eb = echestblue.get()
    val ea = echestalpha.get()
    val chest = Color(r, g, b, a)
    val echest = Color(er, eg, eb, ea)

    private fun getColor(tileEntity: ITileEntity):Color?{
        return when {
            chestValue.get() && classProvider.isTileEntityChest(tileEntity) && !clickedBlocks.contains(tileEntity.pos) -> chest
            enderChestValue.get() && classProvider.isTileEntityEnderChest(tileEntity) && !clickedBlocks.contains(tileEntity.pos) -> echest
            else -> null
        }
    }
