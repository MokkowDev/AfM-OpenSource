...
class Scaffold : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf("Normal", "Rewinside", "Expand", "AAC", "HypixelTest"), "Normal")
    ...
    // Autoblock
    private val autoBlockValue = ListValue("AutoBlock", arrayOf("Off", "AutoPick", "Spoof", "Switch"), "Spoof")
...
// UPDATE EVENTS
    ...
    private fun onUpdate(event: UpdateEvent) {
        ...
        if (mc.thePlayer!!.onGround) {
            when (modeValue.get().toLowerCase()) {
                "hypixeltest" -> {
                    autoBlockValue.set("Spoof")
                    rotationModeValue.set("StaticYaw")
                    sameYValue.set(true)
                    safeWalkValue.set(false)
                    speedModifierValue.set(0.84)
                    timerValue.set(1.1)
                    eagleValue.set("Off")
                ...
                "aac" -> {
                    ...
                    rotationModeValue.set("StaticYaw")
                    autoBlockValue.set("Switch")
                    timerValue.set(1.0f)
                    modeValue.set("Normal")
                    eagleValue.set("Silent")
                }
                "rewinside" -> {
    ...
    fun onRender2D(event: Render2DEvent) {
        if (counterDisplayValue.get()) {
            ...
            val info = "§a$blocksAmount"
            val scaledResolution = classProvider.createScaledResolution(mc)

            RenderUtils.drawBorderedRect(
                scaledResolution.scaledWidth / 1 - 1.toFloat(),
                scaledResolution.scaledHeight / 1 + 1.toFloat(),
                scaledResolution.scaledWidth / 1 + Fonts.font40.getStringWidth(info) + 2.toFloat(),
                scaledResolution.scaledHeight / 1 + 1.toFloat(), 3f, Color.TRANSLUCENT, Color.TRANSLUCENT
            )
...
// SCAFFOLD VISUALS
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        for (i in 0 until if (modeValue.get().equals("Expand", true)) expandLengthValue.get() + 1 else 2) {
            ...
            val placeInfo: PlaceInfo? = PlaceInfo.get(blockPos)
            if (isReplaceable(blockPos) && placeInfo != null) {
                RenderUtils.drawBlockBox(blockPos, Color(0, 255, 0, 200), false)
                break
            }
        }
    }
    ...
    override val tag: String
        get() = "${modeValue.get()} | ${maxDelayValue.get()} | ${minDelayValue.get()}"
}