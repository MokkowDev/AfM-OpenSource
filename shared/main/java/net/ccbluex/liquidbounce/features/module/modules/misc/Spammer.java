...
public class Spammer extends Module {
    ...
    private final TextValue messageValue = new TextValue("Message", LiquidBounce.CLIENT_NAME + " on top | still in beta | TheMosKau on top sub to him");
    ...
    public void onUpdate(UpdateEvent event) {
        if(msTimer.hasTimePassed(delay)) {
            mc.getThePlayer().sendChatMessage(customValue.get() ? replace(messageValue.get()) : messageValue.get() + " >" + RandomUtils.randomString(4 + new Random().nextInt(4)) + "<");
            msTimer.reset();
            delay = TimeUtils.randomDelay(minDelayValue.get(), maxDelayValue.get());
        }
    }
