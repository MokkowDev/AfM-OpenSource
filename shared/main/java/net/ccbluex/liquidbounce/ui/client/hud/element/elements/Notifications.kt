...
class Notifications(x: Double = 0.0, y: Double = 30.0, scale: Float = 1F,
     ...
     */
    private val exampleNotification = Notification("Please add me :<")
...
class Notification(private val message: String) {
    ...
    init {
        textLength = Fonts.minecraftFont.getStringWidth(message)
    }
    ...
    fun drawNotification() {
        // Draw notification
        RenderUtils.drawRect(-x + 8 + textLength, 0F, -x, -20F, Color(0, 0, 0, 95).rgb)
        Fonts.minecraftFont.drawString(message, -x + 4, -14F, Int.MAX_VALUE)
